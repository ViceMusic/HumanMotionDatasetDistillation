...
"""
Protocol-corrected unofficial HDT reproduction for ETT + DLinear, with random-position concatenation baseline.
...
class Config:
    ...
    # Random baseline variant:
    # Instead of one contiguous subsequence of length M,
    # sample several short contiguous pieces from random positions and concatenate them to length M.
    random_concat_num_pieces: int = 16  # 384 / 16 = 24 points per piece for ETTh1
    ...
    distilled_save_name: str = "hdt_ETTh1_DLinear_M384_random_concat_baseline.npz"
...
def sample_random_position_concat(series_np, total_length, num_pieces):
    """
    Random baseline variant.

    Instead of selecting one contiguous subsequence:
        S = X[start : start + M]

    this samples several short contiguous pieces from random positions:
        piece_1 = X[s1 : s1 + piece_len]
        piece_2 = X[s2 : s2 + piece_len]
        ...
        S = concat(piece_1, piece_2, ...)

    The final S has the same length M = total_length.

    This deliberately breaks long-range temporal continuity while preserving
    local short-term patterns inside each piece.
    """
    n = series_np.shape[0]
    if num_pieces <= 0:
        raise ValueError("num_pieces must be positive")

    piece_len = int(np.ceil(total_length / num_pieces))
    if n < piece_len:
        raise ValueError("Cannot sample piece length {} from series length {}".format(piece_len, n))

    pieces = []
    starts = []
    for _ in range(num_pieces):
        start = random.randint(0, n - piece_len)
        starts.append(start)
        pieces.append(series_np[start:start + piece_len])

    concat = np.concatenate(pieces, axis=0)[:total_length]
    return torch.from_numpy(concat.astype(np.float32)).float(), starts, piece_len
...
def main():
    ...
    print("========== HDT ETT DLinear: Random Position-Concatenation Baseline ==========")
    ...
    if cfg.run_random:
        print("\n========== Random Position-Concatenation Baseline ==========")
        random_S, random_starts, random_piece_len = sample_random_position_concat(
            train_np,
            total_length=cfg.synthetic_len,
            num_pieces=cfg.random_concat_num_pieces,
        )
        print(
            "Random concat: M={}, num_pieces={}, piece_len={}, first_starts={}".format(
                cfg.synthetic_len,
                cfg.random_concat_num_pieces,
                random_piece_len,
                random_starts[:10],
            )
        )
        ...
        random_test = evaluate_windows(
            ...
            *make_all_windows(test_np, cfg.input_len, cfg.pred_len),
            cfg=cfg,
            desc="RandomConcat-Test",
        )
        results["random_concat_val"] = random_val
        results["random_concat_test"] = random_test
