...
'''
这个脚本的目标是做点小验证
siMLPe full GM 是否真的不稳定？
last-layer GM / raw / HDT / pred-vel / acc 这些信号里，
有没有某些组合在训练过程中更容易被 scheduler 选中？
'''
...
class DistillConfig:
    ...
    # bandit-style adaptive action scheduler
    # A2/A3: choose GM parameter scope
    # A4/A5: choose optional motion regularization
    # A6/A7: choose raw or HDT-filtered sequence path
    adaptive_actions: bool = True
    scheduler_epsilon: float = 0.15
    scheduler_ema: float = 0.90
    gm_last_n_params: int = 4

    window_mode: str = "random"
    seed: int = 888
    print_interval: int = 10

    save_name: str = "h36m_expmap_sequences_distilled_simlpe_adaptive_actions_bs16_iter8000.npz"
...
class TrainConfig:
    ...
    log_path: str = "logs/train_simlpe_adaptive_actions_bs16_iter8000.txt"
...
def acceleration_smoothness_loss(series):
    """
    Penalize abrupt second-order changes in the synthetic motion sequence.
    series: [B, T, 99]
    """
    if series.shape[1] < 3:
        return torch.zeros((), device=series.device)
    vel = series[:, 1:] - series[:, :-1]
    acc = vel[:, 1:] - vel[:, :-1]
    return acc.pow(2).mean()


def select_gm_params(backbone, gm_scope="full", last_n_params=4):
    """
    full: use all trainable parameters.
    last: use only the last several trainable parameter tensors as a cheap approximation
          of last-layer/head gradient matching.

    This is intentionally robust to unknown SiMLPe internal names.
    """
    named_params = [(name, p) for name, p in backbone.named_parameters() if p.requires_grad]
    if not named_params:
        raise ValueError("No trainable parameters found in backbone.")

    if gm_scope == "full":
        return [p for _name, p in named_params]

    if gm_scope == "last":
        last_n_params = max(1, min(int(last_n_params), len(named_params)))
        return [p for _name, p in named_params[-last_n_params:]]

    raise ValueError("Unknown gm_scope: {}".format(gm_scope))


def compute_gradient_matching_loss_single(
    backbone,
    real_past,
    real_future,
    syn_past,
    ...
    gm_scope="full": match gradients of all trainable SiMLPe parameters.
    gm_scope="last": match gradients of the last N trainable tensors only.
    """
    params = select_gm_params(backbone, gm_scope=gm_scope, last_n_params=last_n_params)
...
class AdaptiveActionScheduler:
    """
    A light bandit-style scheduler, not full RL.

    It chooses one action from each group:
      - GM scope: A2 last-layer GM or A3 full GM
      - regularization: none, A4 pred+vel, or A5 acceleration
      - sequence path: A6 raw or A7 HDT-filtered

    Reward is the step-to-step decrease of the observed proxy loss.
    The same reward is credited to all actions selected in that step.
    """

    def __init__(self, epsilon=0.15, ema=0.90):
        self.epsilon = float(epsilon)
        self.ema = float(ema)

        self.groups = {
            "gm": ["A2_simlpe_last_layer_gm", "A3_simlpe_full_gm"],
            "reg": ["none", "A4_pred_vel_regularization", "A5_acceleration_regularization"],
            "seq": ["A6_raw_sequence", "A7_hdt_filtered_sequence"],
        }
        self.scores = {a: 0.0 for actions in self.groups.values() for a in actions}
        self.counts = {a: 0 for actions in self.groups.values() for a in actions}
        self.prev_proxy = None

    def _choose_from_group(self, actions):
        if random.random() < self.epsilon:
            return random.choice(actions)
        max_score = max(self.scores[a] for a in actions)
        best = [a for a in actions if self.scores[a] == max_score]
        return random.choice(best)

    def select(self):
        gm_action = self._choose_from_group(self.groups["gm"])
        reg_action = self._choose_from_group(self.groups["reg"])
        seq_action = self._choose_from_group(self.groups["seq"])

        return {
            "gm_action": gm_action,
            "reg_action": reg_action,
            "seq_action": seq_action,
            "gm_scope": "last" if gm_action == "A2_simlpe_last_layer_gm" else "full",
            "use_hdt_filter": seq_action == "A7_hdt_filtered_sequence",
        }

    def update(self, action, proxy_loss):
        proxy_loss = float(proxy_loss)
        if self.prev_proxy is None:
            reward = 0.0
        else:
            reward = self.prev_proxy - proxy_loss

        self.prev_proxy = proxy_loss

        for key in ["gm_action", "reg_action", "seq_action"]:
            a = action[key]
            self.counts[a] += 1
            old = self.scores[a]
            self.scores[a] = self.ema * old + (1.0 - self.ema) * reward
    ...
    def short_status(self):
        ...
        for group_name, actions in self.groups.items():
            body = ", ".join(
                "{}:{:.4g}/{}".format(a, self.scores[a], self.counts[a])
                for a in actions
            )
            parts.append("{}[{}]".format(group_name, body))
        return " | ".join(parts)
...
def train_distillation():
    ...
    scheduler = AdaptiveActionScheduler(
        epsilon=cfg.scheduler_epsilon,
        ema=cfg.scheduler_ema,
    )
    ...
    print("lambda_vel:", cfg.lambda_vel, "lambda_pred:", cfg.lambda_pred, "lambda_acc:", cfg.lambda_acc)
    print("Adaptive actions:", cfg.adaptive_actions, "epsilon:", cfg.scheduler_epsilon, "ema:", cfg.scheduler_ema)
    print("GM last N params:", cfg.gm_last_n_params)
    ...
    for step in range(1, cfg.outer_steps + 1):
        action = scheduler.select() if cfg.adaptive_actions else {
            "gm_action": "A3_simlpe_full_gm",
            "reg_action": "A4_pred_vel_regularization",
            "seq_action": "A6_raw_sequence",
            "gm_scope": "full",
            "use_hdt_filter": cfg.use_hdt_filter,
        }
        ...
        for i in range(real_sub_batch.shape[0]):
            ...
            grad_losses = [
                compute_gradient_matching_loss_single(
                    ...
                    syn_future,
                    output_len,
                    gm_scope=action["gm_scope"],
                    last_n_params=cfg.gm_last_n_params,
                )
            ...
            l_acc_syn = torch.zeros((), device=device)

            if action["reg_action"] == "A4_pred_vel_regularization":
            ...
            elif action["reg_action"] == "A5_acceleration_regularization":
                l_acc_syn = acceleration_smoothness_loss(syn_h)
            ...
            per_seq_accs.append(cfg.lambda_acc * l_acc_syn.detach())
        ...
        reward = scheduler.update(action, total_loss.detach().cpu().item()) if cfg.adaptive_actions else 0.0
        ...
        row = {
            ...
            "L_acc_syn": float(torch.stack(per_seq_accs).mean().detach().cpu()),
            "L_total": float(total_loss.detach().cpu()),
            "reward": float(reward),
            "gm_action": action["gm_action"],
            "reg_action": action["reg_action"],
            "seq_action": action["seq_action"],
            "syn_mean": syn_mean,
            "syn_std": syn_std,
            "syn_min": syn_min,
            "syn_max": syn_max,
        ...
        if step % cfg.print_interval == 0:
            print(
                ...
                "L_acc_syn={L_acc_syn:.6f} "
                "L_total={L_total:.6f} "
                "reward={reward:.6f} "
                "gm={gm_action} reg={reg_action} seq={seq_action} "
            ...
            if cfg.adaptive_actions:
                print("scheduler:", scheduler.short_status())
