...
# ============================================================
# Motion-Aware Anchor Multi-Window GM distillation + train
# Save as:
#   methods/smoke_test/run_sequence_maam_gm_distill_and_train.py
#
# Run:
#   python methods/smoke_test/run_sequence_maam_gm_distill_and_train.py
#
# Core rule:
...
class DistillConfig:
    ...
    # MAAM-GM: Motion-Aware Anchor Multi-window Gradient Matching
    # Each real/synthetic sequence pair samples multiple windows per update.
    num_windows_per_sequence: int = 4
    ...
    # Anchor keeps synthetic sequences near their real-crop initialization.
    # Acceleration suppresses unnatural second-order temporal jitter.
    ...
    save_name: str = "h36m_expmap_sequences_distilled_sequence_maam_gm_bs16_iter8000.npz"
...
class TrainConfig:
    ...
    log_path: str = "logs/train_sequence_maam_gm_bs16_iter8000.txt"
...
class SequenceFrequencySyntheticMotionBank(nn.Module):
    ...
    def __init__(self, seq_infos, synthetic_len=100, feature_dim=99, init_motion=None, init_std=0.02):
        ...
        self.register_buffer("init_motion", init_motion.clone().detach())
    ...
    def get_init(self, keys):
        ids = torch.tensor(
            [self.key_to_idx[str(key)] for key in keys],
            device=self.init_motion.device,
            dtype=torch.long,
        )
        return self.init_motion[ids]
...
def acceleration_loss_expmap(series):
    if series.shape[1] < 3:
...
def anchor_loss_expmap(syn_sub, init_sub):
    return F.mse_loss(syn_sub, init_sub.detach())
...
def train_distillation():
    ...
    print("lambda_anchor:", cfg.lambda_anchor, "lambda_acc:", cfg.lambda_acc)
    print("num_windows_per_sequence:", cfg.num_windows_per_sequence)
    ...
    for step in range(1, cfg.outer_steps + 1):
        ...
        for i in range(real_sub_batch.shape[0]):
            ...
            real_past, real_future = make_windows_from_subseries(
                ...
                input_len,
                output_len,
                num_windows=cfg.num_windows_per_sequence,
            ...
            l_anchor = anchor_loss_expmap(syn_sub, init_sub)
            l_acc = acceleration_loss_expmap(syn_sub)
            ...
            per_seq_anchors.append(cfg.lambda_anchor * l_anchor.detach())
            per_seq_accs.append(cfg.lambda_acc * l_acc.detach())
        ...
        row = {
            ...
            "L_anchor": float(torch.stack(per_seq_anchors).mean().detach().cpu()),
            "L_acc": float(torch.stack(per_seq_accs).mean().detach().cpu()),
