...
class DistillConfig:
    ...
    lambda_priv_trend: float = 0.5
    lambda_priv_vel: float = 0.5
    ...
    save_name: str = "h36m_expmap_sequences_distilled_priv_future_stgraph_g01_pt05_pv05_sp5_tmp5_stv5_bs64_iter8000.npz"
...
class TrainConfig:
    ...
    log_path: str = "logs/train_priv_future_stgraph_g01_pt05_pv05_sp5_tmp5_stv5_bs64_iter8000.txt"
...
def save_sequence_bank_npz(
    ...
    np.savez(
        ...
        distill_method=np.array("privileged_future_stgraph_guided_motion_distillation_weighted_aux", dtype=object),
...
def train_distillation():
    ...
    for step in range(1, cfg.outer_steps + 1):
        ...
        row = {
            ...
            "aux_total": float((
                torch.stack(per_seq_trends).mean()
                + torch.stack(per_seq_vels).mean()
                + torch.stack(per_seq_spatials).mean()
                + torch.stack(per_seq_temporals).mean()
                + torch.stack(per_seq_st_vels).mean()
            ).detach().cpu()),
        ...
        if step % cfg.print_interval == 0:
            print(
                ...
                "L_spatial={L_spatial:.6f} "
                "L_temporal={L_temporal:.6f} "
                "L_st_vel={L_st_vel:.6f} "
                "L_total={L_total:.6f} "
                "aux_total={aux_total:.6f} "
...
def train_and_evaluate(train_npz_path):
    ...
    acc_log.write("Seed : {}\n".format(cfg.seed))
    acc_log.write("Method : privileged_future_stgraph_guided_motion_distillation_weighted_aux\n")
