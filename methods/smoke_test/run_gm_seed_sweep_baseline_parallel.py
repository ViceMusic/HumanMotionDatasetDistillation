...
"""
Baseline seed sweep for Human3.6M / siMLPe.

Experiments in this script:
  1) random100_baseline      x seeds [888, 999, 1234, 2026, 3407]
  2) middle100_baseline      x seeds [888, 999, 1234, 2026, 3407]
  3) uniform_piece5x20_baseline x seeds [888, 999, 1234, 2026, 3407]
...
No distillation is performed: the selected initialization segments are saved directly and then used for train/eval.
Experiments are launched in parallel processes.

Save as:
  methods/smoke_test/run_gm_seed_sweep_baseline_parallel.py
Run:
  python methods/smoke_test/run_gm_seed_sweep_baseline_parallel.py
"""
...
class GlobalConfig:
    ...
    output_dir: str = "datasets/gm_seed_sweep_parallel_baseline"
    log_root: str = "logs/gm_seed_sweep_parallel_baseline"
    summary_log: str = "logs/gm_seed_sweep_parallel_baseline/summary_mean_std_baseline.txt"
...
BASELINE_EXPERIMENTS = [
    {
        "base_name": "random100_baseline",
        "segment_method": "random100",
        "desc": "Seed sweep baseline: save the random100 initialization directly without GM, then train/evaluate siMLPe.",
    },
    {
        "base_name": "middle100_baseline",
        "segment_method": "middle100",
        "desc": "Seed sweep baseline: save the middle100 initialization directly without GM, then train/evaluate siMLPe.",
    },
    {
        "base_name": "uniform_piece5x20_baseline",
        "segment_method": "uniform_piece5x20",
        "desc": "Seed sweep baseline: save the uniform_piece5x20 initialization directly without GM, then train/evaluate siMLPe.",
    },
]
...
def write_summary(summary_path, results):
    ...
    ordered_bases = [base["base_name"] for base in BASELINE_EXPERIMENTS]
    ...
    with open(summary_path, "w") as f:
        f.write("Baseline seed sweep summary for Human3.6M / siMLPe.\n")
        f.write("This run evaluates three initialization baselines over five seeds. No GM/distillation is performed.\n")
        f.write("Each individual log begins with a short description of the experiment and stores full distillation/train output.\n")
        f.write("Baseline setting: selected 100-frame initialization segments are saved directly, then siMLPe is trained/evaluated with the same train configuration.\n")
        f.write("RESULT_KEYS: {}\n".format(RESULT_KEYS))
