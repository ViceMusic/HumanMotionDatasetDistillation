...
# Variant: DL-MSweep-Len100-BatchMix; synthetic_len=100; random_50pieces removed; TrainConfig.num_workers=1; SweepConfig.max_workers=45.
...
#   2. Keep the sequence-level synthetic bank, but compute the distillation loss on the whole sampled batch.
...
class DistillConfig:
    ...
    output_dir: str = "datasets/dlinear_msweep_len100_batchmix"
...
def train_distillation():
    ...
    for step in range(1, cfg.outer_steps + 1):
        ...
        loss_harm_log = cfg.lambda_harm * l_harm.detach()
        loss_grad_log = cfg.lambda_grad * l_grad.detach()
        loss_pred_log = cfg.lambda_pred * l_pred_syn.detach()
        loss_vel_log = cfg.lambda_vel * l_vel_syn.detach()
        ...
        row = {
            "step": step,
            "L_harm": float(loss_harm_log.detach().cpu()),
            "L_grad": float(loss_grad_log.detach().cpu()),
            "L_pred_syn": float(loss_pred_log.detach().cpu()),
            "L_vel_syn": float(loss_vel_log.detach().cpu()),
...
def train_and_evaluate(train_npz_path, run_name):
    ...
    print("Train batches are sequence-pure for train/eval; distillation loss is batch-mixed across sequences.")
...
# logic above, but runs multiple seeds with max_workers=45 and adds:
#   - full
...
@dataclass
class SweepConfig:
    name: str = "DL-MSweep-Len100-BatchMix"
    seeds = (888, 999, 1234, 2026, 3407)
    output_dir: str = "datasets/dlinear_msweep_len100_batchmix"
    log_root: str = "logs/dlinear_msweep_len100_batchmix"
    summary_log: str = "logs/dlinear_msweep_len100_batchmix/summary_mean_std.txt"
    max_workers: int = 45
...
def train_distillation_variant(seed, num_pieces, lambda_pred, lambda_vel, save_path):
    ...
    # Batch-mixed GM protocol uses random contiguous real sequence crop.
...
def write_sweep_summary(summary_path, results):
    ...
    with open(summary_path, "w") as f:
        f.write("DL-MSweep-Len100-BatchMix summary for Human3.6M / DLinear.\n")
        f.write("This run keeps the original DLinear protocols and runs multiple seeds with max_workers=45.\n")
        f.write("Full uses the full-data global-shuffle protocol; compressed/random/GM runs use the original sequence-pure compressed-bank protocol.\n")
