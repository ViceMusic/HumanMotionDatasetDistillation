# Auto-generated variant based on Pasted code(120).py
# Variant: DL-MSweep-In20-Out10-Len40-BatchMix; synthetic_len=40; random_50pieces removed; TrainConfig.num_workers=1; SweepConfig.max_workers=45.
# Batch-mixed distillation: True
...
#      - random_complete: one contiguous 40-frame crop
#      - random_20pieces: 20 random pieces concatenated into 40 frames
#      #   4. Train/evaluate DLinear on distilled + selected random baselines.
...
DLINEAR_INPUT_LEN = 20
DLINEAR_OUTPUT_LEN = 10


@dataclass
class DistillConfig:
    data_path: str = "/home/user/workspace/HumanMotionDatasetDistillation/datasets/processed/Human3.6m/h36m_expmap_sequences.npz"
    output_dir: str = "datasets/dlinear_msweep_in20_out10_len40_batchmix"

    synthetic_len: int = 40
    feature_dim: int = 99
    top_k: int = 16
    ...
    save_name: str = "h36m_expmap_sequences_distilled_sequence_dlinear_raw_g01_bs16_iter8000.npz"
...
def train_distillation():
    ...
    input_len = DLINEAR_INPUT_LEN
    output_len = DLINEAR_OUTPUT_LEN
    ...
    for step in range(1, cfg.outer_steps + 1):
        ...
        per_seq_harms = [cfg.lambda_harm * l_harm.detach()]
        per_seq_grads = [cfg.lambda_grad * l_grad.detach()]
        per_seq_preds = [cfg.lambda_pred * l_pred_syn.detach()]
        per_seq_vels = [cfg.lambda_vel * l_vel_syn.detach()]
        harmonic_counts = [harmonic_count]
...
def evaluate_mpjpe_dlinear(model, npz_path, subjects, config, device):
    ...
    train_step_len = DLINEAR_OUTPUT_LEN
...
# ============================================================
# DLinear multi-seed plus runner
# Name: DL-MSweep+
# This section keeps the original DLinear distillation / random-baseline
# logic above, but uses DLinear 20->10, synthetic_len=40, batch-mixed GM, max_workers=45 and adds:
#   - full
#   - pure GM: lambda_pred=lambda_vel=0
#   - strong-motion GM: lambda_pred=lambda_vel=300
# ============================================================


@dataclass
class SweepConfig:
    name: str = "DL-MSweep-In20-Out10-Len40-BatchMix"
    seeds = (888, 999, 1234, 2026, 3407)
    output_dir: str = "datasets/dlinear_msweep_in20_out10_len40_batchmix"
    log_root: str = "logs/dlinear_msweep_in20_out10_len40_batchmix"
    summary_log: str = "logs/dlinear_msweep_in20_out10_len40_batchmix/summary_mean_std.txt"
    max_workers: int = 45
...
EXPERIMENTS = [
    {
        ...
        "lambda_pred": None,
        "lambda_vel": None,
        "desc": "Full-data baseline. Train DLinear on the original full Human3.6M training data. Uses the same full-data training protocol: global shuffled windows.",
    },
    {
        ...
        "lambda_pred": 150.0,
        "lambda_vel": 150.0,
        "desc": "Original DLinear GM. Random contiguous real crop initialization; DLinear GM + pred/vel regularization with lambda_pred=lambda_vel=150.",
    },
    {
        ...
        "lambda_pred": 0.0,
        "lambda_vel": 0.0,
        "desc": "Pure DLinear GM. Random contiguous real crop initialization; only gradient matching is used, lambda_pred=lambda_vel=0.",
    },
    {
        "base_name": "distilled_dlinear_gm_strong_motion",
        "mode": "gm",
        "num_pieces": 1,
        "lambda_pred": 300.0,
        "lambda_vel": 300.0,
        "desc": "Strong-motion DLinear GM. Random contiguous real crop initialization; DLinear GM + stronger pred/vel regularization with lambda_pred=lambda_vel=300.",
    },
    {
        ...
        "lambda_pred": None,
        "lambda_vel": None,
        "desc": "Random baseline. One random contiguous 40-frame crop per training sequence; no distillation.",
    },
    {
        ...
        "lambda_pred": None,
        "lambda_vel": None,
        "desc": "Random baseline. 20 random pieces concatenated into 40 frames per training sequence; no distillation.",
    },
]
...
def build_sweep_experiments():
    ...
    for exp in EXPERIMENTS:
        for seed in sweep_cfg.seeds:
            item = dict(exp)
            item["seed"] = int(seed)
            item["name"] = "{}_seed{}".format(exp["base_name"], seed)
            out.append(item)
    return out


def get_initial_subseries_by_pieces(sampler, num_pieces):
    subs = []
    for info in sampler.seq_infos:
        seq = sampler.by_key[info["key"]]
        subs.append(sample_random_concatenated(seq, sampler.synthetic_len, num_pieces=num_pieces))
    return torch.tensor(np.stack(subs), dtype=torch.float32)


def train_distillation_variant(seed, num_pieces, lambda_pred, lambda_vel, save_path):
    cfg = DistillConfig()
    cfg.seed = int(seed)
    cfg.lambda_pred = float(lambda_pred)
    cfg.lambda_vel = float(lambda_vel)
    ...
    # Original protocol for DLinear GM uses random contiguous real sequence crop.
    # num_pieces=1 is the original initialization. The helper also allows pieces if needed.
    init_motion = get_initial_subseries_by_pieces(sampler, num_pieces=num_pieces)
    ...
    print("========== Distillation: DLinear GM Variant ==========")
    print("Seed:", cfg.seed)
    print("Device:", device)
    print("Num training sequences:", len(sampler.keys))
    print("Input length:", input_len, "Output length:", output_len)
    print("Initialization num_pieces:", num_pieces)
    print("lambda_harm:", cfg.lambda_harm, "lambda_grad:", cfg.lambda_grad)
    print("lambda_vel:", cfg.lambda_vel, "lambda_pred:", cfg.lambda_pred)
    print("use_hdt_filter:", cfg.use_hdt_filter, "top_k:", cfg.top_k)
    print("Backbone: DLinear")
...
def create_random_baseline_npz_variant(seed, num_pieces, name, save_path):
    cfg = DistillConfig()
    cfg.seed = int(seed)
    set_all_seeds(cfg.seed)
    ...
    save_motion_npz(
        ...
        heldout_entries=sampler.get_heldout_entries(),
        step=0,
        logs=[{"baseline": name, "num_pieces": num_pieces, "synthetic_len": cfg.synthetic_len, "seed": cfg.seed}],
        feature_dim=cfg.feature_dim,
        tag=name,
    )
    print("Saved random baseline {} to {}".format(name, save_path))
...
def train_and_evaluate_variant(train_npz_path, run_name, seed, log_dir, full_global_shuffle=False):
    cfg = TrainConfig()
    cfg.seed = int(seed)
    cfg.log_dir = log_dir
    ...
    if full_global_shuffle:
        # This matches the separate first-round full-data DLinear script:
        # global shuffled windows, not sequence-pure batches.
        train_loader = DataLoader(
            train_dataset,
            batch_size=cfg.batch_size,
            shuffle=True,
        ...
        batch_info = "Global shuffled DataLoader"
        num_batches = len(train_loader)
    else:
        # This is the original compressed-bank protocol in this script:
        ...
        batch_info = "Sequence-pure batch sampler"
        num_batches = len(train_batch_sampler)

    print("========== Train/Eval DLinear: {} ==========".format(run_name))
    print("Seed:", cfg.seed)
    print("Train NPZ:", train_npz_path)
    print("Batch protocol:", batch_info)
    print("Batch size:", cfg.batch_size, "num_batches:", num_batches, "total_iters:", cfg.total_iters)

    log_dir_abs = resolve_path(cfg.log_dir)
    ensure_dir(log_dir_abs)
    log_path = os.path.join(log_dir_abs, "train_eval_dlinear_{}.txt".format(run_name))
    acc_log = open(log_path, "w")
    acc_log.write("Run : {}\n".format(run_name))
    acc_log.write("Seed : {}\n".format(cfg.seed))
    acc_log.write("Backbone : DLinear\n")
    ...
    while nb_iter < cfg.total_iters:
        for past_expmap, future_expmap in train_loader:
            ...
            if nb_iter % cfg.print_every == 0:
                msg = "[{}] train iter {} loss={:.6f}".format(run_name, nb_iter, avg_loss / cfg.print_every)
...
def run_one_sweep_experiment(exp):
    sweep_cfg = SweepConfig()
    base_cfg = DistillConfig()

    seed = int(exp["seed"])
    output_dir = resolve_path(sweep_cfg.output_dir)
    log_root = resolve_path(sweep_cfg.log_root)
    ensure_dir(output_dir)
    ensure_dir(log_root)

    exp_log_path = os.path.join(log_root, "{}.log".format(exp["name"]))
    exp_npz_path = os.path.join(output_dir, "h36m_{}_{}.npz".format(exp["name"], base_cfg.synthetic_len))

    with open(exp_log_path, "w", buffering=1) as log_f:
        with redirect_stdout(log_f), redirect_stderr(log_f):
            print("Suite:", sweep_cfg.name)
            print("Experiment:", exp["name"])
            print("Description:", exp["desc"])
            print("Base experiment:", exp["base_name"])
            print("Mode:", exp["mode"])
            print("Seed:", seed)
            print("Num pieces:", exp["num_pieces"])
            print("lambda_pred:", exp["lambda_pred"])
            print("lambda_vel:", exp["lambda_vel"])
            ...
            try:
                if exp["mode"] == "full":
                    train_npz_path = base_cfg.data_path
                    ...
                    metrics = train_and_evaluate_variant(
                        ...
                        seed,
                        log_root,
                        full_global_shuffle=True,
                    )
                elif exp["mode"] == "baseline":
                    train_npz_path = create_random_baseline_npz_variant(
                        seed,
                        exp["num_pieces"],
                        exp["base_name"],
                        exp_npz_path,
                    ...
                    metrics = train_and_evaluate_variant(
                        ...
                        seed,
                        log_root,
                        full_global_shuffle=False,
                    )
                elif exp["mode"] == "gm":
                    train_npz_path = train_distillation_variant(
                        seed,
                        exp["num_pieces"],
                        exp["lambda_pred"],
                        exp["lambda_vel"],
                ...
                result = {
                    ...
                    "seed": seed,
                    "mode": exp["mode"],
                    "num_pieces": exp["num_pieces"],
                    "lambda_pred": exp["lambda_pred"],
                    "lambda_vel": exp["lambda_vel"],
                    "metrics": metrics,
...
def write_sweep_summary(summary_path, results):
    ensure_dir(os.path.dirname(summary_path))
    ordered_bases = [exp["base_name"] for exp in EXPERIMENTS]
    grouped = {base: [] for base in ordered_bases}
    for r in results:
        grouped.setdefault(r.get("base_name", r.get("name")), []).append(r)

    with open(summary_path, "w") as f:
        f.write("DL-MSweep-In20-Out10-Len40-BatchMix summary for Human3.6M / DLinear.\n")
        f.write("This run uses DLinear input_len=20, output_len=10, synthetic_len=40, batch-mixed distillation, and max_workers=45.\n")
        f.write("Full uses the full-data global-shuffle protocol; compressed/random train/eval runs keep the original sequence-pure compressed-bank protocol; GM distillation uses batch-mixed loss.\n")
        f.write("Added GM variants: pure GM with lambda_pred=lambda_vel=0, and strong-motion GM with lambda_pred=lambda_vel=300.\n")
        f.write("RESULT_KEYS: {}\n".format(RESULT_KEYS))
        f.write("Seeds: {}\n".format(list(SweepConfig().seeds)))
        f.write("\n")

        f.write("Per-run results:\n")
        base_order = {name: i for i, name in enumerate(ordered_bases)}
        for r in sorted(results, key=lambda x: (base_order.get(x.get("base_name"), 999), int(x.get("seed") or 0))):
            f.write("{}\n".format(r.get("name")))
            f.write("  status: {}\n".format(r.get("status")))
            f.write("  base_name: {}\n".format(r.get("base_name")))
            f.write("  seed: {}\n".format(r.get("seed")))
            f.write("  mode: {}\n".format(r.get("mode")))
            f.write("  num_pieces: {}\n".format(r.get("num_pieces")))
        ...
        for base in ordered_bases:
            ...
            f.write("{}\n".format(base))
            f.write("  num_ok: {} / {}\n".format(len(ok), len(SweepConfig().seeds)))
            if ok:
...
def main():
    ...
    sweep_cfg = SweepConfig()
    ensure_dir(resolve_path(sweep_cfg.log_root))
    ensure_dir(resolve_path(sweep_cfg.output_dir))
    ensure_dir(os.path.dirname(resolve_path(sweep_cfg.summary_log)))

    experiments = build_sweep_experiments()
    summary_path = resolve_path(sweep_cfg.summary_log)

    print("Running {} experiments for {} with max_workers={}".format(len(experiments), sweep_cfg.name, sweep_cfg.max_workers))
    print("Seeds:", list(sweep_cfg.seeds))
    print("Summary log:", summary_path)

    results = []
    with ProcessPoolExecutor(max_workers=sweep_cfg.max_workers) as executor:
        futures = {executor.submit(run_one_sweep_experiment, exp): exp for exp in experiments}
    ...
    name_order = {exp["name"]: i for i, exp in enumerate(experiments)}
    results = sorted(results, key=lambda r: name_order.get(r.get("name"), 999))
    write_sweep_summary(summary_path, results)
