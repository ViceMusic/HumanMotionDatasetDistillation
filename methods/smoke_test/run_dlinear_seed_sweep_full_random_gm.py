...
class GlobalConfig:
    ...
    output_dir: str = "datasets/dlinear_seed_sweep_full_random_gm"
    log_root: str = "logs/dlinear_seed_sweep_full_random_gm"
    summary_log: str = "logs/dlinear_seed_sweep_full_random_gm/summary_mean_std.txt"
...
def train_and_evaluate(train_npz_path, run_name, seed):
    cfg = TrainConfig()
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    ...
    train_dataset = H36MExpmapWindowDataset(train_npz_path, cfg.train_subjects, input_len, output_len, sample_rate=cfg.sample_rate)
    ...
    print("Adaptive batch sampler: batch_size <= {} num_batches = {}".format(cfg.batch_size, len(train_batch_sampler)))
    print("Seed:", seed)
    ...
    with open(exp_log_path, "w", buffering=1) as log_f:
        with redirect_stdout(log_f), redirect_stderr(log_f):
            ...
            try:
                ...
                metrics = train_and_evaluate(train_npz_path, exp["name"], seed)
...
def write_summary(summary_path, results):
    ...
    with open(summary_path, "w") as f:
        ...
        f.write("This run evaluates full, DLinear GM, pure DLinear GM, random_complete, random_20pieces, and random_20pieces_gm over multiple seeds.\n")
