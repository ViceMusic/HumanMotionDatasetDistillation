...
"""
MAP-GM: Middle-Anchored Piece Gradient Matching for Human3.6M / siMLPe.

Core experiment:
  S = learnable synthetic sequence initialized by uniform_piece10x10.
  L_total = GM(S, real_piece10x10) + pred/vel(S) + lambda_middle_res * ||S - middle100||^2

Here piece10x10 provides the learnable synthetic body, while middle100 is used as a
per-iteration residual / motion-manifold anchor. The final output is optimized S,
not a post-hoc mixture of middle100 and piece10x10.

This script keeps the same train/eval settings as the segment-selection GM script.
It also runs middle100 and uniform_piece10x10 baselines for comparison.

Save as:
  methods/smoke_test/run_map_gm_middle_anchor.py
Run:
  python methods/smoke_test/run_map_gm_middle_anchor.py
"""
...
class GlobalConfig:
    ...
    output_dir: str = "datasets/map_gm_middle_anchor"
    log_root: str = "logs/map_gm_middle_anchor"
    summary_log: str = "logs/map_gm_middle_anchor/summary_results.txt"

    synthetic_len: int = 100
    feature_dim: int = 99
    seed: int = 888

    # Parallelism. Start conservatively if you see CUDA OOM.
    max_workers: int = 3
...
EXPERIMENTS = [
    {"name": "middle100", "segment_method": "middle100", "use_gm": False, "middle_anchor": False,
     "desc": "Baseline: each training sequence is replaced by the middle contiguous 100 frames; then siMLPe is trained/evaluated."},

    {"name": "uniform_piece10x10", "segment_method": "uniform_piece10x10", "use_gm": False, "middle_anchor": False,
     "desc": "Baseline: each training sequence is replaced by 10 uniformly spaced chronological pieces, each of length 10, concatenated to 100 frames; then siMLPe is trained/evaluated."},

    {"name": "map_gm_piece10x10_middle_anchor", "segment_method": "uniform_piece10x10", "use_gm": True, "middle_anchor": True,
     "desc": "MAP-GM: initialize learnable synthetic S with uniform_piece10x10, optimize S with siMLPe GM + pred/vel regularization, and add middle100 residual anchor loss at every distillation step: lambda_middle_res * ||S - middle100||^2. Final output is optimized S, not a post-hoc mixture."},
]
...
def train_distillation_piece_with_middle_anchor(pool, save_path, gcfg, global_cfg, seed):
    """
    MAP-GM.

    Learnable synthetic S is initialized from uniform_piece10x10.
    The real GM target also uses uniform_piece10x10.
    middle100 is used only as a residual / motion-manifold anchor at every iter:

        L_total = GM(S, real_piece10x10)
                + pred/vel(S)
                + lambda_middle_res * MSE(S, middle100)

    The saved dataset is the optimized S.
    """
    set_seed(seed)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    simlpe_config = build_simlpe_config()
    ...
    piece_init, _piece_metas = pool.build_initial_segments("uniform_piece10x10", global_cfg.synthetic_len)
    middle_anchor, _middle_metas = pool.build_initial_segments("middle100", global_cfg.synthetic_len)
    middle_anchor = middle_anchor.to(device)

    key_to_idx = {info["key"]: idx for idx, info in enumerate(pool.seq_infos)}

    bank = SequenceFrequencySyntheticMotionBank(
        pool.seq_infos,
        synthetic_len=global_cfg.synthetic_len,
        feature_dim=global_cfg.feature_dim,
        init_motion=piece_init,
    ).to(device)
    bank.project_valid_rfft()
    ...
    print("[MAP-GM] synthetic init: uniform_piece10x10")
    print("[MAP-GM] GM real target: uniform_piece10x10")
    print("[MAP-GM] middle anchor: middle100")
    print("[MAP-GM] save path:", save_path)
    print("[MAP-GM] device:", device)
    print("[MAP-GM] input_len/output_len:", input_len, output_len)
    print("[MAP-GM] lambda_grad/pred/vel/middle_res:",
          gcfg.lambda_grad, gcfg.lambda_pred, gcfg.lambda_vel, gcfg.lambda_middle_res)
    ...
    for step in range(1, gcfg.outer_steps + 1):
        ...
        anchor_batch = []
        for key in keys:
            # deterministic piece target for the current original sequence
            seg, _ = sample_segment(pool.by_key[str(key)], global_cfg.synthetic_len, "uniform_piece10x10")
            real_batch.append(seg)
            anchor_batch.append(middle_anchor[key_to_idx[str(key)]])

        real_sub_batch = torch.tensor(np.stack(real_batch), dtype=torch.float32, device=device)
        middle_sub_batch = torch.stack(anchor_batch, dim=0).to(device)
        syn_sub_batch = bank(keys)
        ...
        for i in range(real_sub_batch.shape[0]):
            real_sub = real_sub_batch[i:i + 1]
            syn_sub = syn_sub_batch[i:i + 1]
            mid_sub = middle_sub_batch[i:i + 1]
            ...
            # middle100 residual / anchor intervention at every iter
            l_middle_res = F.mse_loss(syn_sub, mid_sub)

            loss_i = (
                gcfg.lambda_grad * l_grad
                + gcfg.lambda_pred * l_pred_syn
                + gcfg.lambda_vel * l_vel_syn
                + gcfg.lambda_middle_res * l_middle_res
            )
            per_seq_losses.append(loss_i)
            per_seq_grads.append(gcfg.lambda_grad * l_grad.detach())
            per_seq_preds.append(gcfg.lambda_pred * l_pred_syn.detach())
            per_seq_vels.append(gcfg.lambda_vel * l_vel_syn.detach())
            per_seq_res.append(gcfg.lambda_middle_res * l_middle_res.detach())
        ...
        with torch.no_grad():
            ...
            row = {
                ...
                "L_pred_syn": float(torch.stack(per_seq_preds).mean().detach().cpu()),
                "L_vel_syn": float(torch.stack(per_seq_vels).mean().detach().cpu()),
                "L_middle_res": float(torch.stack(per_seq_res).mean().detach().cpu()),
                "L_total": float(total_loss.detach().cpu()),
                "syn_mean": float(syn_time.mean().detach().cpu()),
                "syn_std": float(syn_time.std().detach().cpu()),
                "syn_min": float(syn_time.min().detach().cpu()),
        ...
        if step % gcfg.print_interval == 0:
            print(
                "distill step {step} L_grad={L_grad:.6f} L_pred_syn={L_pred_syn:.6f} "
                "L_vel_syn={L_vel_syn:.6f} L_middle_res={L_middle_res:.6f} "
                "L_total={L_total:.6f} syn_mean={syn_mean:.6f} "
                "syn_std={syn_std:.6f} example_key={example_key}".format(**row),
                flush=True,
            )
    ...
    save_bank_npz(
        ...
        logs,
        heldout_entries=pool.get_heldout_entries(),
        init_method="map_gm_piece10x10_middle_anchor",
        feature_dim=global_cfg.feature_dim,
    )
    print("[MAP-GM] Saved optimized S to", save_path)
    return save_path
...
def run_one_experiment(exp):
    ...
    # Keep deterministic but avoid identical random100 crops across process if needed.
    seed = int(global_cfg.seed + abs(hash(exp["name"])) % 100000)
    ...
    with open(exp_log_path, "w", buffering=1) as log_f:
        with redirect_stdout(log_f), redirect_stderr(log_f):
            ...
            try:
                ...
                if exp["use_gm"] and exp.get("middle_anchor", False):
                    train_npz_path = train_distillation_piece_with_middle_anchor(
                        pool=pool,
...
def write_summary(summary_path, results):
    ...
    with open(summary_path, "w") as f:
        f.write("MAP-GM middle-anchor experiment summary.\n")
        f.write("Each individual experiment log begins with a short description of the experiment.\n")
        f.write("Baseline experiments train siMLPe directly on fixed 100-frame segment selections.\n")
        f.write("MAP-GM initializes S from uniform_piece10x10, applies siMLPe GM + pred/vel regularization, and adds middle100 residual anchor loss at every distillation step before train/eval.\n")
        f.write("RESULT_KEYS: {}\n".format(RESULT_KEYS))
        f.write("\n")
        for r in results:
            f.write("{}\n".format(r.get("name")))
