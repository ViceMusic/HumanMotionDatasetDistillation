...
"""
Segment-selection baselines + GM refinement for Human3.6M / siMLPe.
...
Experiments in this script:
  ...
  8) last100 + GM
  9) uniform_piece5x20
 10) uniform_piece5x20 + GM
 11) uniform_piece10x10
 12) uniform_piece10x10 + GM

Each experiment writes an individual log under LOG_ROOT, and the final summary is
written to SUMMARY_LOG. Experiments are launched in parallel processes.

Save as:
  methods/smoke_test/run_segment_selection_gm_parallel.py
Run:
  python methods/smoke_test/run_segment_selection_gm_parallel.py
"""
...
class GlobalConfig:
    ...
    output_dir: str = "datasets/segment_selection_gm_parallel"
    log_root: str = "logs/segment_selection_gm_parallel"
    summary_log: str = "logs/segment_selection_gm_parallel/summary_results.txt"
...
EXPERIMENTS = [
    {"name": "first100", "segment_method": "first100", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by its first contiguous 100 frames; then siMLPe is trained/evaluated."},
    {"name": "first100_gm", "segment_method": "first100", "use_gm": True,
     "desc": "GM refinement: initialize each synthetic sequence with first100, optimize it with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},

    {"name": "random100", "segment_method": "random100", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by one random contiguous 100-frame crop; this matches the original contiguous_100 baseline."},
    {"name": "random100_gm", "segment_method": "random100", "use_gm": True,
     "desc": "GM refinement: initialize each synthetic sequence with random100, optimize it with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},

    {"name": "middle100", "segment_method": "middle100", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by the middle contiguous 100 frames; then siMLPe is trained/evaluated."},
    {"name": "middle100_gm", "segment_method": "middle100", "use_gm": True,
     "desc": "GM refinement: initialize each synthetic sequence with middle100, optimize it with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},

    {"name": "last100", "segment_method": "last100", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by its last contiguous 100 frames; then siMLPe is trained/evaluated."},
    {"name": "last100_gm", "segment_method": "last100", "use_gm": True,
     "desc": "GM refinement: initialize each synthetic sequence with last100, optimize it with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},

    {"name": "uniform_piece5x20", "segment_method": "uniform_piece5x20", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by 20 uniformly spaced chronological pieces, each of length 5, concatenated to 100 frames."},
    {"name": "uniform_piece5x20_gm", "segment_method": "uniform_piece5x20", "use_gm": True,
     "desc": "GM refinement: initialize with uniform_piece5x20, optimize with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},

    {"name": "uniform_piece10x10", "segment_method": "uniform_piece10x10", "use_gm": False,
     "desc": "Baseline: each training sequence is replaced by 10 uniformly spaced chronological pieces, each of length 10, concatenated to 100 frames."},
    {"name": "uniform_piece10x10_gm", "segment_method": "uniform_piece10x10", "use_gm": True,
     "desc": "GM refinement: initialize with uniform_piece10x10, optimize with siMLPe GM + pred/vel regularization, then train/evaluate siMLPe."},
]
...
def write_summary(summary_path, results):
    ...
    with open(summary_path, "w") as f:
        f.write("Segment-selection baselines and GM refinement summary.\n")
        ...
        f.write("+GM experiments initialize the synthetic bank from the same selected segments, then run siMLPe gradient matching with pred/vel regularization before train/eval.\n")
        f.write("RESULT_KEYS: {}\n".format(RESULT_KEYS))
        f.write("\n")
        for r in results:
            f.write("{}\n".format(r.get("name")))
