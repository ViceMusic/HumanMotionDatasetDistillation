...
# Save as:
#   methods/smoke_test/run_random_sequence_methods_and_train.py
#
# Run:
#   python methods/smoke_test/run_random_sequence_methods_and_train.py
#
...
class RandomBuildConfig:
    ...
    save_prefix: str = "h36m_expmap_sequences_random_len100"
...
def sample_contiguous(seq, out_len):
    ...
    return seq[start:start + out_len].copy(), {"start": start, "mode": "contiguous"}
...
def build_random_motion_for_method(seq, method, out_len):
    if method == "contiguous_100":
        return sample_contiguous(seq, out_len)
